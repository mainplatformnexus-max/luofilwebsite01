...
import { Plus, Trash2, Edit, Trophy, Signal } from "lucide-react";
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { cn } from "@/lib/utils";
import { useSportContent } from "@/hooks/useFirestore";
import { sportContentService, FirestoreSportContent } from "@/lib/firestore";
import { toast } from "@/hooks/use-toast";

const categories = ["Football", "Basketball", "Tennis", "Cricket", "Rugby", "Athletics", "Boxing", "Swimming", "Cycling", "General"];

const emptyForm = {
  title: "", slug: "", thumbnail: "", streamUrl: "", category: "Football",
  country: "", isLive: false, isVip: false, order: 1, description: "", tags: "",
};

export default function AdminSports() {
  const { sports, loading } = useSportContent();
  const [formOpen, setFormOpen] = useState(false);
  const [editing, setEditing] = useState<FirestoreSportContent | null>(null);
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ ...emptyForm, order: sports.length + 1 });
    setFormOpen(true);
  };

  const openEdit = (s: FirestoreSportContent) => {
    setEditing(s);
    setForm({
      title: s.title, slug: s.slug, thumbnail: s.thumbnail, streamUrl: s.streamUrl,
      category: s.category, country: s.country, isLive: s.isLive, isVip: s.isVip,
      order: s.order, description: s.description || "", tags: s.tags?.join(", ") || "",
    });
    setFormOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const data: Omit<FirestoreSportContent, "id"> = { ...form, slug, tags };
      if (editing?.id) {
        await sportContentService.update(editing.id, data);
        toast({ title: "Sport content updated" });
      } else {
        await sportContentService.create(data);
        toast({ title: "Sport content added" });
      }
      setFormOpen(false);
    } catch (e: any) {
  ...
  const deleteItem = async (id: string) => {
    if (!confirm("Delete this sport content?")) return;
    await sportContentService.delete(id);
    toast({ title: "Deleted" });
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Trophy className="w-6 h-6 text-amber-400" /> Sports ({sports.length})
          </h1>
          <p className="text-white/50 text-sm mt-1">Manage sports content and live matches</p>
        </div>
        <button
          onClick={openCreate}
      ...
      {loading ? (
        <div className="text-center py-12 text-white/40">Loading sports content...</div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-12 text-white/40">
          <Trophy className="w-12 h-12 mx-auto mb-3 opacity-30" />
          <p>No sports content found. Add your first sport!</p>
        </div>
      ) : (
          ...
          {filtered.map((item) => (
            <div key={item.id} className="bg-white/5 border border-white/10 rounded-xl overflow-hidden group">
              <div className="aspect-video bg-white/10 relative">
                ...
                <div className="absolute top-2 right-2 px-2 py-0.5 bg-black/60 text-white/70 text-[10px] rounded">{item.category}</div>
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                  <button onClick={() => openEdit(item)} className="p-2 bg-blue-500 rounded-lg">
                    <Edit className="w-4 h-4" />
                  </button>
                  <button onClick={() => item.id && deleteItem(item.id)} className="p-2 bg-red-500 rounded-lg">
              ...
              <div className="p-3">
                ...
                <p className="text-xs text-white/40 mt-0.5">{item.country} • Order #{item.order}</p>
                {item.tags && item.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-1">
                    {item.tags.map((t) => (
                      <span key={t} className="text-[10px] px-1.5 py-0.5 bg-white/10 rounded">{t}</span>
                    ))}
                  </div>
                )}
              </div>
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="bg-[#12121a] border-white/10 text-white max-w-md max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Sport Content" : "Add Sport Content"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <label className="text-xs text-white/50 mb-1 block">Title *</label>
              <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className={inp} placeholder="e.g. UEFA Champions League" />
            </div>
            <div>
            ...
            <div>
              <label className="text-xs text-white/50 mb-1 block">Stream URL * (m3u8, mp4, YouTube, or embed)</label>
              <input value={form.streamUrl} onChange={(e) => setForm({ ...form, streamUrl: e.target.value })} className={inp} placeholder="https://..." />
            </div>
            ...
            <div>
              ...
              <input value={form.tags} onChange={(e) => setForm({ ...form, tags: e.target.value })} className={inp} placeholder="Football, Live, HD" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
            ...
            >
              {saving ? "Saving..." : editing ? "Update" : "Add Sport Content"}
            </button>
          </div>
        </DialogContent>
      </Dialog>
