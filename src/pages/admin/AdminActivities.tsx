...
const actionTypes = ["All", "Watched", "Downloaded", "Searched", "Subscribed", "Commented", "Clicked", "Browsed", "Rated", "Signed Up", "Shared"];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-4 flex-wrap">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            placeholder="Search by user, email, phone or target..."
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl overflow-x-auto">
        {loading ? (
          <div className="flex items-center justify-center py-10 gap-2 text-white/40"><Loader2 className="w-4 h-4 animate-spin" /> Loading activities...</div>
        ) : filtered.length === 0 ? (
          <div className="py-10 text-center text-white/30 text-sm">
            {activities.length === 0 ? "No activities recorded yet. User actions will appear here." : "No activities match your search."}
          </div>
        ) : (
              ...
              {filtered.map((a) => {
                ...
                return (
                  <tr key={a.id} className="border-b border-white/5 hover:bg-white/5">
                    <td className="p-3 text-white/50 text-xs whitespace-nowrap">{formatTime(a)}</td>
                    <td className="p-3 font-medium">{a.user}</td>
                    <td className="p-3">
                      {contact ? (
                        <div className="space-y-0.5">
                          {contact.email && <div className="text-xs text-white/60">{contact.email}</div>}
                          {contact.phone && <div className="text-xs text-amber-400/80 font-medium">{contact.phone}</div>}
                        </div>
                      ) : (
                    ...
                    <td className="p-3 text-center">
                      <span className={cn("px-2 py-0.5 rounded-full text-xs", actionColors[a.action] || "bg-white/10 text-white/60")}>{a.action}</span>
                    </td>
                    <td className="p-3 text-white/70">{a.target}</td>
                    <td className="p-3 text-center text-xs text-white/50">{a.device || "—"}</td>
                  </tr>
                );
              })}
            </tbody>
